# TENVI Dashboard

TENVI는 React + Vite 기반의 개인 AI 대시보드입니다. 서버 없이 브라우저 안에서 동작하며, Tasks, Notes, Board, Calendar, Command, Timer, Map, Settings 모듈을 하나의 HUD 스타일 작업 공간으로 제공합니다.

현재 TENVI는 로컬 우선(local-first) 구조입니다. Tasks, Notes, Board, Calendar, Timer, 언어/테마 설정은 `localStorage`에 저장하고, Map 사진 기록과 컬렉션은 IndexedDB에 저장합니다.

## 기술 스택

- React 19
- Vite 8
- JavaScript
- CSS
- localStorage
- IndexedDB
- Vitest
- exifr
- Leaflet
- React Leaflet

## 실행 방법

의존성 설치:

```bash
npm install
```

개발 서버 실행:

```bash
npm run dev
```

프로덕션 빌드:

```bash
npm run build
```

빌드 결과 미리보기:

```bash
npm run preview
```

## 테스트와 검증

테스트 실행:

```bash
npm run test:run
```

전체 확인 시 권장 명령:

```bash
npm run test:run
npm run build
npm run lint
```

현재 테스트는 다음 영역을 다룹니다.

- localStorage key 상수 유지
- `translations` 한국어/영어 key 구조
- Command Console 명령 파싱과 결과 생성
- Timer 시간 포맷과 입력값 정규화
- Board 게시글 생성/수정/삭제/조회수 증가 로직
- Settings 백업 JSON 검증
- Calendar 하루 일정/기간 일정 호환, 날짜 범위, 표시 메타 계산
- Tasks due date 집계
- Map 사진 위치 정규화, draft 생성, 편집 patch 생성
- Map 미리보기 이미지 크기 계산
- 장소 검색 결과 정규화와 검색 범위 처리

## 현재 모듈

### Dashboard

Dashboard는 탭 기반 구조로 전체 요약을 나눕니다.

- 개요: 오늘 브리핑, 핵심 수치, 가장 중요한 최근 항목
- 작업: Tasks와 Calendar 중심 요약
- 기록: Notes와 Map Archive 중심 요약
- 시스템: Timer 완료 세션, 현재 설정, 저장 데이터 현황

Dashboard 탭은 화면 내부 UI 상태로만 관리하며 `localStorage`에 저장하지 않습니다. 시스템 탭은 Settings를 대체하지 않고 현재 상태를 읽기 전용으로 보여줍니다.

### Tasks

- Task 추가
- 완료 상태 변경
- Task 삭제
- 전체/진행 중/완료 필터
- due date 입력
- localStorage 저장

### Notes

- 노트 제목 입력
- 노트 내용 입력
- 노트 추가
- 노트 삭제
- localStorage 저장

### Board

Board는 localStorage 기반의 일반 게시판 모듈입니다.

- 게시글 목록 화면
  - 제목 중심 목록 표시
  - 목록 오른쪽에 작성 날짜 표시
  - 제목 클릭 시 상세 화면 진입
- 게시글 상세 화면
  - 제목, 작성자, 작성 시간, 조회수 표시
  - 내용은 게시글 본문형 레이아웃으로 표시
  - 상세 진입 시 조회수 1 증가
  - 목록으로 이동
  - 수정 화면 진입
  - 게시글 삭제
- 게시글 작성 화면
  - 작성자, 제목, 내용 입력
  - 저장 후 입력 폼 초기화
  - 저장 후 목록 화면으로 이동
- 게시글 수정 화면
  - 기존 작성자, 제목, 내용 값으로 폼 초기화
  - 저장 시 `author`, `title`, `content`, `updatedAt` 갱신
  - `id`, `createdAt`, `views`는 유지
- `tenvi.board.posts` localStorage key 유지
- 게시글 순수 로직은 `src/modules/boardLogic.js`에서 관리

### Calendar

- 월간 달력 UI
- 날짜 선택
- 선택 날짜의 일정 목록 표시
- 선택 날짜에 마감된 Tasks 표시
- 하루 일정 추가/삭제
- 시작일과 종료일을 가진 기간 일정 추가
- 기존 date-only 일정 fallback 지원
- 기존 `date + endDate` 형태 일정 호환
- 기간 일정이 포함된 모든 날짜에 달력 표시
- 기간 일정 시작/중간/끝 날짜 표시 class 계산
- 기간 일정 badge와 날짜 범위 표시
- 달력 범례 표시
- Dashboard와 Command Console에서 Calendar 데이터 사용
- `tenvi.calendar.events` localStorage key 유지

새 일정 저장 시 `date`는 `startDate`와 같은 값으로 유지합니다. 기존 저장 데이터를 강제로 마이그레이션하지 않고, 읽을 때 fallback으로 호환합니다.

### Command

저장된 Tasks, Notes, Calendar 데이터를 기반으로 규칙 기반 명령을 실행합니다. 한국어와 영어 명령을 모두 지원하며, 결과는 현재 언어 설정에 맞춰 표시합니다.

예시 명령:

| 한국어 | English | 설명 |
| --- | --- | --- |
| `상태 분석` | `analyze status` | Tasks, Notes 상태 요약 |
| `미완료 작업` | `show active tasks` | 미완료 Task 목록 표시 |
| `완료율` | `completion rate` | Task 완료율 표시 |
| `최근 노트` | `recent notes` | 최근 노트 목록 표시 |
| `노트 검색 [키워드]` | `search notes [keyword]` | 노트 제목/내용 검색 |
| `도움말` | `help` | 사용 가능한 명령 표시 |
| `오늘 추천` | `recommend task` | 미완료 Task 중 하나 추천 |
| `태스크 검색 [키워드]` | `search tasks [keyword]` | Task 제목 검색 |
| `데이터 상태` | `data status` | 저장 데이터 상태 표시 |
| `타이머 열기` | `open timer` | Timer 모듈로 이동 |
| `설정 열기` | `open settings` | Settings 모듈로 이동 |
| `집중 모드` | `focus mode` | Task 추천 후 Timer 모듈로 이동 |
| `오늘 마감` | `today tasks` | 오늘 마감된 미완료 Tasks 표시 |

Calendar 명령:

| 한국어 | English | 설명 |
| --- | --- | --- |
| `오늘 일정` | `today schedules` | 오늘 일정 목록 표시 |
| `이번 달 일정` | `this month schedules` | 이번 달 일정 요약 표시 |
| `일정 검색 [키워드]` | `search schedules [keyword]` | 일정 제목/메모 검색 |
| `다음 일정` | `next schedule` | 오늘 이후 가장 가까운 일정 표시 |
| `일정 상태` | `schedule status` | 전체/오늘/이번 달 일정 상태 표시 |

### Timer

- Focus/Break 타이머
- 시작, 일시정지, 초기화
- Focus/Break 시간 설정
- 완료 세션 수 localStorage 저장
- Stopwatch
- Lap 기록 추가/초기화

### Map

Map 모듈은 사진 위치 기반 기록을 지도에 표시하는 로컬 사진 아카이브입니다. 사진 기록과 컬렉션은 IndexedDB에 저장합니다.

주요 기능:

- 탐색
- 사진 업로드
- 컬렉션 관리

탐색 화면:

- 컬렉션 select 필터
- 텍스트 사진 검색
- 위치 방식 필터
- 필터 적용 badge
- 사진 기록 목록
- 지도 마커 표시
- 사진 목록 또는 지도 마커 선택 시 상세 패널 표시
- 사진 미선택 시 compact 필터 요약 표시
- 전체 위치 보기

사진 기록 기능:

- 사진 단일/다중 등록
- EXIF GPS 위치 정보 자동 추출
- GPS가 없는 사진은 지도 클릭으로 위치 지정
- 장소 검색 결과 선택으로 위치 지정
- 위치 지정 방식 표시: `exif`, `manual`, `search`, `unknown`
- 제목과 메모 입력
- 사진 기록 편집과 삭제
- 새로고침 후 IndexedDB에서 기록 복원

장소 검색:

- 공개 Nominatim Search API 사용
- 검색 버튼 또는 Enter 입력 시에만 요청
- 입력 중 자동완성 요청 없음
- 최대 5개 결과 표시
- 동일 검색어/언어/범위 조합 메모리 캐시
- 검색 범위: 전체, 일본, 한국
- OpenStreetMap/Nominatim 출처 표시
- Google Places, Google Maps Platform 같은 유료/과금 가능 서비스는 사용하지 않음

Map 저장 정책:

- 원본 고해상도 사진은 저장하지 않음
- 화면 표시용 리사이즈 미리보기 Blob만 IndexedDB에 저장
- 파일명, 파일 형식, 촬영 시각, 좌표, 위치 지정 방식, 제목, 메모, 생성/수정 시각 저장
- localStorage에는 Map 사진 기록이나 이미지 데이터를 저장하지 않음

### Settings

- 언어 변경
- 기본 시작 모듈 선택
- HUD 효과 선택
  - 한국어: 기본, 줄임
  - 영어: Normal, Reduced
- 테마 선택
  - HUD
  - Standard
- 저장 데이터 현황 표시
  - Tasks
  - Notes
  - Calendar 일정
  - Map 사진 기록
- Tasks/Notes 데이터 초기화
  - 명시적인 확인 후 실행
- JSON 백업 내보내기
- JSON 백업 복원
  - 복원 전 확인
  - 대상 데이터 검증
  - 실패 시 가능한 범위에서 롤백

현재 백업/복원 대상:

- Tasks
- Notes
- Calendar 일정
- Timer 완료 세션
- 언어 설정
- 기본 시작 모듈
- HUD 효과 설정
- 테마 설정
- Map 사진 기록
- Map 컬렉션

## UI/UX 정리 현황

- 공통 버튼, 입력창, 카드, 패널, 빈 상태 스타일 정리
- HUD 모드와 일반 모드 대비 보정
- Dashboard 탭 기반 정보 구조 정리
- Calendar 선택 날짜, 목록, 입력 영역 구분
- Calendar 기간 일정 bar, badge, 범례 표시
- Settings 일반 설정, 백업/복원, 초기화 위험도 구분
- Map 탐색 화면 정돈
  - 컬렉션 필터 select화
  - 검색/위치 필터 적용 상태 처리
  - 사진 목록 공간 정리
  - 사진 미선택 요약 compact화
  - 지도 중심성을 높이도록 패널 폭 조정

## 데이터 저장 구조

TENVI는 별도 서버 없이 브라우저 저장소를 사용합니다. 기존 key와 저장 구조는 사용자 데이터와 직접 연결되므로 변경하지 않는 것을 원칙으로 합니다.

### localStorage

| 데이터 | localStorage key | 설명 |
| --- | --- | --- |
| Tasks | `todo-manager-lite.todos` | Task 목록, 완료 상태, due date |
| Notes | `tenvi.notes` | 노트 제목과 내용 |
| Board | `tenvi.board.posts` | 게시글 작성자, 제목, 내용, 조회수 |
| Calendar | `tenvi.calendar.events` | 하루 일정과 기간 일정 |
| Timer 세션 | `tenvi.timer.completedSessions` | 완료된 Focus 세션 수 |
| 언어 설정 | `tenvi.language` | `ko` 또는 `en` |
| 기본 시작 모듈 | `tenvi.startModule` | 앱 시작 시 열 모듈 |
| HUD 효과 | `tenvi.hudEffect` | `normal` 또는 `reduced` |
| 테마 | `tenvi.theme` | `hud` 또는 `standard` |

Calendar 일정 데이터 구조:

```js
{
  id: string,
  date: 'YYYY-MM-DD',
  startDate: 'YYYY-MM-DD',
  endDate: 'YYYY-MM-DD',
  title: string,
  memo: string,
  createdAt: string,
  updatedAt: string
}
```

Calendar 호환 규칙:

- 실제 시작일: `event.startDate || event.date`
- 실제 종료일: `event.endDate || event.startDate || event.date`
- 기존 date-only 일정은 하루 일정으로 표시
- 기존 `date + endDate` 일정은 `date`를 시작일로 사용하는 기간 일정으로 처리
- 저장된 기존 데이터를 강제로 마이그레이션하지 않음
- 새 일정 저장 시 `date`는 `startDate`와 같은 값 유지

Board 게시글 데이터 구조:

```js
{
  id: string,
  title: string,
  content: string,
  author: string,
  category: string,
  createdAt: string,
  updatedAt: string,
  views: number
}
```

Board 호환 규칙:

- 기존 저장 글에 `author`가 없으면 화면에서 기본 작성자 fallback을 사용
- 기존 저장 글에 `views`가 없으면 상세 진입 시 1부터 시작
- 빈 제목 또는 빈 내용은 새 글/수정 저장 대상에서 제외

### IndexedDB

Map 사진 기록과 컬렉션은 IndexedDB에 저장합니다.

| 항목 | 값 |
| --- | --- |
| DB 이름 | `tenvi-photo-archive` |
| 사진 Object store | `photoRecords` |
| 컬렉션 Object store | `photoCollections` |
| keyPath | `id` |

사진 기록 데이터 구조:

```js
{
  id: string,
  previewImageBlob: Blob,
  previewImageMimeType: string,
  previewImageWidth: number,
  previewImageHeight: number,
  originalFileName: string,
  fileType: string,
  takenAt: string,
  latitude: number,
  longitude: number,
  locationSource: 'exif' | 'manual' | 'search' | 'unknown',
  collectionId: string | null,
  title: string,
  memo: string,
  createdAt: string,
  updatedAt: string
}
```

컬렉션 데이터 구조:

```js
{
  id: string,
  name: string,
  description: string,
  startDate: string,
  endDate: string,
  createdAt: string,
  updatedAt: string
}
```

## 프로젝트 구조

```text
tenvi-dashboard/
├─ public/
├─ src/
│  ├─ components/
│  │  └─ Sidebar.jsx
│  ├─ constants/
│  │  ├─ storageKeys.js
│  │  └─ storageKeys.test.js
│  ├─ i18n/
│  │  ├─ translations.js
│  │  └─ translations.test.js
│  ├─ modules/
│  │  ├─ Board.jsx
│  │  ├─ boardLogic.js
│  │  ├─ boardLogic.test.js
│  │  ├─ bulkPhotoUploadLogic.js
│  │  ├─ bulkPhotoUploadLogic.test.js
│  │  ├─ Calendar.jsx
│  │  ├─ calendarLogic.js
│  │  ├─ calendarLogic.test.js
│  │  ├─ Command.jsx
│  │  ├─ commandLogic.js
│  │  ├─ commandLogic.test.js
│  │  ├─ Dashboard.jsx
│  │  ├─ Map.jsx
│  │  ├─ mapLogic.js
│  │  ├─ mapLogic.test.js
│  │  ├─ Notes.jsx
│  │  ├─ Settings.jsx
│  │  ├─ settingsBackup.js
│  │  ├─ settingsBackup.test.js
│  │  ├─ Tasks.jsx
│  │  ├─ tasksLogic.js
│  │  ├─ tasksLogic.test.js
│  │  ├─ Timer.jsx
│  │  ├─ timerLogic.js
│  │  └─ timerLogic.test.js
│  ├─ services/
│  │  ├─ photoArchiveBackupService.js
│  │  ├─ photoArchiveRepository.js
│  │  ├─ photoArchiveSummaryService.js
│  │  ├─ photoCollectionRepository.js
│  │  ├─ placeSearchService.js
│  │  └─ placeSearchService.test.js
│  ├─ utils/
│  │  ├─ imageUtils.js
│  │  └─ imageUtils.test.js
│  ├─ App.css
│  ├─ App.jsx
│  ├─ index.css
│  └─ main.jsx
├─ AGENTS.md
├─ package.json
├─ package-lock.json
├─ vite.config.js
└─ README.md
```

## 개발 원칙

- 기능은 가능한 모듈 단위로 관리합니다.
- 모듈 전환은 React state 기반이며 React Router를 사용하지 않습니다.
- 외부 라이브러리는 기본적으로 추가하지 않습니다.
- 비용이 발생하거나 결제 계정/API 과금 설정이 필요한 외부 API는 현재 범위에서 사용하지 않습니다.
- 기존 localStorage key와 IndexedDB 구조를 유지합니다.
- 기존 저장 데이터는 명시적인 사용자 확인 없이 초기화하지 않습니다.
- UI는 TENVI HUD 톤을 유지하되, 일반 모드에서도 즉시 읽히는 사용성을 우선합니다.
- 한국어/영어 UI 문구는 `translations` 구조를 통해 관리합니다.
- 저장소 로직은 화면 로직과 분리합니다.
- Board 게시글 생성/수정/삭제/조회수 같은 순수 로직은 `boardLogic.js`에서 관리합니다.

주요 저장소/서비스:

- Board 게시글 생성/수정/삭제/조회수 증가: `src/modules/boardLogic.js`
- Map 사진 기록 저장/조회/수정/삭제: `src/services/photoArchiveRepository.js`
- Map 컬렉션 저장/조회/수정/삭제: `src/services/photoCollectionRepository.js`
- Map 백업 직렬화와 복원 준비: `src/services/photoArchiveBackupService.js`
- Dashboard Map 요약: `src/services/photoArchiveSummaryService.js`
- 장소 검색: `src/services/placeSearchService.js`
- 이미지 미리보기 생성: `src/utils/imageUtils.js`

## 향후 개선 후보

- Board 상세 댓글 기능
- Board 검색/정렬/페이지네이션
- Board 이미지 첨부
- Calendar 일정 수정 기능
- Calendar 반복 일정 기능
- Calendar 기간 막대 다중 행 UI
- Command Console 명령 자동완성
- Map 기록 날짜/여행별 보기 고도화
- Map 검색과 컬렉션 UX 추가 개선
- 실제 AI API 연동
- 사용자별 대시보드 분리

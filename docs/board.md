...
- 카테고리별 게시글 수가 카테고리 버튼마다 직접 표시되는 UI는 현재 코드 기준 확인되지 않습니다. 전체/검색 결과 수와 휴지통 수는 표시됩니다.
...
휴지통은 별도 localStorage key를 쓰지 않고 게시글의 `deletedAt` 필드로 구분합니다.
...
1. 삭제는 `deletedAt`을 추가해 휴지통으로 이동
2. 복구는 `deletedAt` 제거
